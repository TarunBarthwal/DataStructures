...
public:
	...
	void printDisjointSets(vector<graphNode> allNodes)
	{
		...
		int counter = 0;
		for (graphNode current = allNodes[counter]; counter < allNodes.size(); counter++)
		{
			assert (current != NULL);
				
			if(isPresent(mark,&current))
				continue;
			else
				mark.push_back(NULL);
			...
			while(!nodeStack.empty())
			{
				...
				{
					if(isPresent(mark,&current.links[i])) // CHECK
						continue;
		...
		cout<<endl<<"Number of Disjoin Sets = "<<numOfDisjointSets;
